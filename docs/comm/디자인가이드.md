...
|![예시 이미지](https://github.com/muabe/Minor-League/blob/master/docs/comm/img/ratio.png)|
| :-----------: |
...
## 이미지 영역 지정
 1. 기본적으로 아이콘/이미지는 Fit하게 영역 지정(일반적인 방법)
 
 1. ```Click/Touch``` 가 이루어지는 아이콘/이미지에 대해서는 ```터치 영역을 고려하여``` 이미지 영역 지정
    | X | O |
 ...
 1. 아이콘/이미지가 레이아웃 구성상 여백을 포함 할수 있을때는 여백 포함  
...
<br>
 ...
 1. 데이터에 따라 이미지가 바뀌더라도 영역 전체가 이미지가 될 수 있으면 여백까지 포함
    | X | O |
...
1. 데이터 영역인지에 따라 이미지 구분을 달리함 
    | X | O |
    | ![x](https://github.com/muabe/Minor-League/blob/master/docs/comm/img/changeopng) | ![o](https://github.com/muabe/Minor-League/blob/master/docs/comm/img/changeor.ng) |
    | :-----------: | :-----------: |
