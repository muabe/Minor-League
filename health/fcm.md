...
1. 알림 메세지 : SDK(OS)에서 자동 처리됨
 ...
 - 앱이 떠 있을땐 OS별로 각각 아래를 호출함
   ...
   자바스크립트에서는 onMessage()  
   >  note 프로토컬 종류 https://firebase.google.com/docs/reference/fcm/rest/v1/projects.messages?hl=ko#Notification
...
전송 옵션 2가지
 디바이스가 메세지를 받을수 없는 상황일때
 FCM에서 메세지를 누적하고 있다가 받을수 있는 상황으로 되었을때 처리하는 방법
 ** FCM은 전송 순서를보장하지 않는다.
1.비축소형 메시지
